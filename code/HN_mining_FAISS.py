...
def main():
    ...
    def get_HNs(index,model, ds,numb_easy_negatives_per_line, numb_hard_negatives_per_line):
        """
        ...
        for i, data in enumerate(ds):
            ...
            for inx in inxs:
                if inx == -1: break
                if corpus[inx] not in data['positive'] and corpus[inx] != data['anchor']:
                    filtered_inx.append(inx)

            if len(filtered_inx) > numb_hard_negatives_per_line:
                filtered_inx = random.sample(filtered_inx, numb_hard_negatives_per_line)
    ...
    train_dataset=get_HNs(index,model,train_dataset,numb_easy_negatives_per_line=numb_easy_negatives_per_line,numb_hard_negatives_per_line=total_negatives_per_line)
    eval_dataset=get_HNs(index,model,eval_dataset,numb_easy_negatives_per_line=numb_easy_negatives_per_line,numb_hard_negatives_per_line=total_negatives_per_line)
